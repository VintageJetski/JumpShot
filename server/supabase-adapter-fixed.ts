...
import { processPlayerData, mapPlayerRole, isPlayerIGL } from './player-data-utils';
  ...
  async getPlayersForEvent(eventId: number = 1): Promise<PlayerRawStats[]> {
    try {
      ...
      for (const player of playerData) {
        try {
          // Process the player data using our utility function
          const playerRawStats = processPlayerData(player);
